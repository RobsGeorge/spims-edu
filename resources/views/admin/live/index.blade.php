...
<p class="small spims-text-dim mb-3">{{ __('live.zoom_production_keys') }}</p>
...
<x-card variant="panel" tag="form" method="POST" action="{{ route('admin.live.recurrence', $offering) }}" class="mb-4">
    @csrf
...
<x-card variant="panel" class="mb-3">
    ...
    <ul class="small">
        @foreach($session->attendance as $row)
            <li>{{ $row->student->email }} — <x-badge :value="$row->status" /> ({{ $row->minutes_attended }}m)</li>
        @endforeach
    </ul>
