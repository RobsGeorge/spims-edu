...
<p><x-badge :value="$assessment->mode" /> · {{ $assessment->time_limit_minutes }} min</p>
...
<ul>@foreach($assessment->attempts as $a)<li>{{ $a->student->email }} #{{ $a->attempt_no }} <x-badge :value="$a->status" /> {{ $a->total_score }} @if($a->terminated_for_cheating)<span class="badge bg-danger">terminated</span>@endif <a href="{{ route('admin.attempts.proctor', $a) }}">proctor log</a></li>@endforeach</ul>
@endsection
