...
@if($status->value === 'GRADING_LOCKED')
<x-card variant="panel" tag="form" method="POST" action="{{ route('admin.offering-closing.grace', $offering) }}" class="mb-4">
    @csrf
    ...
    @empty
        <p class="spims-text-dim mb-0">{{ __('completion.no_results') }}</p>
    @endforelse
...
<table class="table table-sm">
    ...
    @empty
        <tr><td colspan="3" class="spims-text-dim">{{ __('completion.no_results') }}</td></tr>
    @endforelse
    </tbody>
</table>
</div>
