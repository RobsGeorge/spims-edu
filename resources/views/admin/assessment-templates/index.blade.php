...
<x-card variant="panel" class="mb-4">
    ...
    <form method="POST" action="{{ route('admin.assessment-templates.store') }}">
        ...
        <p class="small spims-text-dim">{{ __('academics.weights_hint') }}</p>
                ...
                <div class="col-md-4">
                    <select name="components[{{ $i }}][kind]" class="form-select" required>
                        @foreach(\App\Enums\ComponentKind::cases() as $kind)
                            ...
                            <option value="{{ $kindVal }}" @selected($kindVal === $defaults[1])>{{ $kindVal }}</option>
                        @endforeach
                    </select>
                </div>
...
@foreach($templates as $template)
    <x-card variant="panel" class="mb-3">
        <h2 class="h6">{{ $template->name }} @if($template->is_default)<span class="badge text-bg-info">{{ __('academics.is_default') }}</span>@endif</h2>
        <ul class="mb-0">
            @foreach($template->components as $c)
                <li>{{ $c->name }} (<x-badge :value="$c->kind" />) — {{ $c->weight_percent }}%</li>
            @endforeach
        </ul>
    </x-card>
@endforeach
