    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('admin.assessment-templates.store') }}">
            ...
            <div class="row g-2 mb-2">
                <div class="col-12 col-md-6"><input name="name" class="form-control" placeholder="{{ __('academics.name') }}" required></div>
                <div class="col-12 col-md-3 form-check mt-2"><input type="checkbox" name="is_default" value="1" class="form-check-input" id="is_default"><label for="is_default" class="form-check-label">{{ __('academics.is_default') }}</label></div>
            </div>
            ...
            @foreach([['Exam', 'EXAM', 60], ['Attendance', 'ATTENDANCE', 20], ['Assignments', 'ASSIGNMENT', 20]] as $i => $defaults)
                <div class="row g-2 mb-2">
                    <div class="col-12 col-md-4"><input name="components[{{ $i }}][name]" class="form-control" value="{{ $defaults[0] }}" required></div>
                    ...
                    <div class="col-12 col-md-4"><input type="number" step="0.01" name="components[{{ $i }}][weight_percent]" class="form-control" value="{{ $defaults[2] }}" required></div>
                </div>
            @endforeach
