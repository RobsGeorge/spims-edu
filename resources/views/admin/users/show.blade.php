...
<div class="people-dossier animate-in">
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            ...
            <section class="app-card p-3 mb-4">
                <h2 class="h6 page-title">{{ __('people.profile_title') }}</h2>
        ...
        <div class="col-lg-6">
            <section class="app-card p-3 mb-4">
                ...
                @if(!empty($capabilities['assign_roles']) && $assignableRoles !== [])
                    <form method="POST" action="{{ route('admin.users.roles.store', $person) }}" class="row g-2 align-items-end">
                        @csrf
                        <div class="col-md-8">
    ...
    <section class="app-card p-3 mb-4">
        <h2 class="h6 page-title">{{ __('people.hold_title') }}</h2>
        <p class="small text-muted-theme">{{ __('people.hold_help') }}</p>
        <p class="mb-3">{{ $hasFinancialHold ? __('people.hold_on') : __('people.hold_off') }}</p>
        @if(!empty($capabilities['enrollment_override']))
            <form method="POST" action="{{ route('admin.enrollments.financial-hold', $person) }}" class="d-flex flex-wrap gap-2">
                ...
                @if($hasFinancialHold)
                    ...
                    <button class="btn btn-outline-secondary btn-sm">{{ __('people.hold_clear') }}</button>
                @else
                    <input type="hidden" name="held" value="1">
                    <button class="btn btn-outline-danger btn-sm">{{ __('people.hold_apply') }}</button>
                @endif
            </form>
        @endif
    </section>
