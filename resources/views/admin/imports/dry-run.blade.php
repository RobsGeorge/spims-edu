...
@php
    ...
    $isMidtermEnrollment = $batch->entity_type === ImportEntityType::MidtermEnrollment;
    $isCredential = $batch->entity_type === ImportEntityType::Credential;
...
@else
    ...
    @elseif($isCourseResult)
        ...
        @if(($report['shadow_courses_created'] ?? 0) > 0 || ($report['shadow_offerings_created'] ?? 0) > 0)
            <div class="row g-3 mb-4">
                <div class="col-6">
        ...
        @elseif($batch->status->value === 'COMMITTED')
            <a href="{{ route('admin.imports.show', $batch) }}" class="btn btn-outline-primary">{{ __('import.batch_title') }}</a>
        @endif
    @elseif($isMidtermEnrollment)
        <div class="row g-3 mb-4">
            <div class="col-6 col-lg-4">
                <x-stat :label="__('import.report_enrollments_created')" :value="$report['enrollments_created'] ?? 0" />
            </div>
            <div class="col-6 col-lg-4">
                <x-stat :label="__('import.report_scores_created')" :value="$report['create'] ?? 0" />
            </div>
            <div class="col-6 col-lg-4">
                <x-stat :label="__('import.report_skip')" :value="$report['skip'] ?? 0" />
            </div>
