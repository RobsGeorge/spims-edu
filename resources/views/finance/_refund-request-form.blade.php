...
<form method="POST" action="{{ route('finance.refund-request', $payment) }}" class="row g-2 align-items-end mt-2">
    ...
    <div class="col-md-3">
        ...
        <input id="refund-amount-{{ $payment->id }}" type="number" name="amount_minor" class="form-control form-control-sm" min="1" max="{{ $payment->amount_minor }}" value="{{ old('amount_minor', $payment->amount_minor) }}" required>
    </div>
    <div class="col-md-4">
