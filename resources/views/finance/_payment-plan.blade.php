...
@if($canCreatePlan)
<form method="POST" action="{{ $action }}" class="card border-0 shadow-sm mb-3">
    @csrf
...
<div class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        ...
        <p class="small mb-2">{{ __('finance.plan_status') }}: {{ $openPlan->status->value }} · {{ $openPlan->installment_count }} × {{ $openPlan->interval->value }}</p>
        ...
        <table class="table table-sm mb-0">
            ...
            @foreach($openPlan->installments as $installment)
                <tr>
                    ...
                    <td>{{ $installment->amount_minor }} {{ $installment->currency->value }}</td>
                    <td>{{ $installment->status->value }}</td>
                    @if($showPayButtons)
                    <td>
