<?php

return [
    'create_year' => 'Create academic year',
    'edit_year' => 'Edit academic year',
    'create_offering' => 'Create offering',
    'edit_offering' => 'Edit offering',
    'edit_semester' => 'Edit semester',
    'update_offering' => 'Update offering',
    'start_date' => 'Start date',
    'end_date' => 'End date',
    'status' => 'Status',
    'registration_start' => 'Registration start',
    'registration_end' => 'Registration end',
    'withdrawal_refund' => 'Withdrawal refund %',
    'add_semester' => 'Add semester',
    'no_semesters' => 'No semesters yet.',
    'mode' => 'Mode',
    'semester' => 'Semester',
    'seat_capacity' => 'Seat capacity',
    'attendance_threshold' => 'Attendance threshold %',
    'clone_week1' => 'Clone with Week 1 scaffold',
    'guide_mode_body' => 'Cohort ties the offering to a semester\'s registration window and calendar. Self-paced students can enroll and progress at any time — no semester needed.',
    'guide_clone_body' => "Despite the name, this doesn't copy another offering's content — it just adds an empty Week 1 container so the content workspace has something to build on.",
    'guide_threshold_body' => 'Overrides the school-wide default attendance threshold for this offering only — the percentage a student must meet to stay in good standing.',
    'assign_staff' => 'Assign staff',
    'pricing' => 'Pricing overrides',
    'resolved' => 'Resolved prices (minor units)',
    'save_pricing' => 'Save pricing',
    'add_week' => 'Add week',
    'add_item' => 'Add item',
    'text_body' => 'Text lesson body',
    'public_preview' => 'Public preview',
    'all_week_titles' => 'All week titles',
    'week_one_content' => 'Week 1 content (open preview)',
    'no_week_one' => 'No Week 1 content yet.',
    'add_drop_week' => 'Add/drop end week',
    'withdrawal_week' => 'Last withdrawal week',
    'year_created' => 'Academic year created.',
    'year_updated' => 'Academic year updated.',
    'semester_created' => 'Semester created.',
    'semester_updated' => 'Semester updated.',
    'offering_created' => 'Offering created.',
    'offering_updated' => 'Offering updated.',
    'staff_assigned' => 'Staff assigned.',
    'staff_removed' => 'Staff removed.',
    'staff_not_assigned' => 'That staff member is not assigned to this offering.',
    'remove_staff' => 'Remove',
    'pricing_saved' => 'Pricing saved.',
    'week_added' => 'Week added.',
    'content_added' => 'Content item added.',
    'cohort_requires_semester' => 'Cohort offerings require a semester.',
    'video_url_invalid' => 'Enter a Vimeo ID, a Vimeo URL, or a YouTube watch / Shorts / embed link.',
    'video_url_unsupported' => 'YouTube playlists and Live links are not supported. Paste a single video link.',
    'video_provider_disabled' => 'This school does not allow :provider videos.',
    'reading_url_invalid' => 'Enter a valid HTTPS link to the reading or file.',
    'reading_url_https' => 'External readings must use HTTPS.',
    'reading_url_host_blocked' => 'That host is not on the school allow-list for readings.',
    'item_type' => 'Type',
    'video_url' => 'Video link',
    'video_url_ph' => 'Vimeo or YouTube URL',
    'vimeo_id' => 'Vimeo ID',
    'external_url' => 'External file URL',
    'external_url_ph' => 'Google Drive, Dropbox, or HTTPS link',
    'upload_file' => 'Upload PDF or image',
    'item_starts_draft' => 'New items stay hidden from students until you publish them.',
    'status_published' => 'Published',
    'status_draft' => 'Draft',
    'publish' => 'Publish',
    'unpublish' => 'Unpublish',
    'delete_item' => 'Delete',
    'delete_item_confirm' => 'Delete this week item?',
    'no_items_in_week' => 'No items in this week yet.',
    'content_updated' => 'Content item updated.',
    'content_deleted' => 'Content item deleted.',
    'content_published' => 'Content item published.',
    'content_unpublished' => 'Content item hidden from students.',
    'upload_type_blocked' => 'Upload a PDF or image (JPG, PNG, WebP, GIF) only.',
    'upload_too_large' => 'That file is larger than :mb MB.',
    'download_disabled' => 'Downloads are turned off for students.',
    'move_up' => 'Move up',
    'move_down' => 'Move down',
    'move_to_week' => 'Move',
    'content_reordered' => 'Item order updated.',
    'content_moved' => 'Item moved to another week.',
    'reorder_invalid' => 'Those items do not all belong to this week.',
    'move_week_invalid' => 'You can only move an item to another week in the same offering.',
    'view_as_student' => 'View as student',
    'preview_banner' => 'Viewing as a student. Locks and week order match a typical enrolled student. Drafts stay hidden.',
    'preview_exit' => 'Exit preview',
    'preview_read_only' => 'Student preview is read-only.',
    'usd_minor' => 'USD (minor units)',
    'egp_minor' => 'EGP (minor units)',
    'minor_units_hint' => 'Enter amounts in minor units (cents/piastres). Example: 2500 = 25.00.',
    'pdf_url' => 'PDF URL',
    'week_label' => 'Week :number',
    'week_number' => 'Week #',
];
