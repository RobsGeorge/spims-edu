...
It is **not yet a Populi-class product**. The domain layer is ahead of the operator UI, the demo seed is a skeleton (no lessons, invoices, exams, attendance, or certificates), and several production integrations (live payments, Zoom, PDF certificates) still degrade to mocks. The next planned phases (S4 completion/credentials, S5 assessment completion) close the largest academic gaps. Matching Populi means adding advising, financial aid, reporting, scheduling, and a finished mobile API after that.
...
| Mobile `/api/v1` | **Foundation only** (auth, me, branding, comms, attendance) |
...
| Mobile API | login / logout / me / branding / announcements / notifications / attendance | Student learn / grades / enroll / pay (S6) and instructor grade/lock (S8) **not built**. |
...
**Roadmap already scheduled (do these next, in order):**

1. **S5 — assessment completion** — proctor escalation that actually terminates; offline assignments; result announcement; assignment dashboard.
2. **S4 — completion + credentials** — criteria engine (grade ∧ attendance), offering close, grace marks, real PDF certificates, student notes.
3. **S6 — student API waves A–E** — phone path in Arabic.
4. **S7 — team projects** — largest new subsystem (11 tables in the plan).
5. **S8 — instructor API**.
6. **S9 — Reverb realtime**, live quiz, campus events (with polling fallback).
...
#### Band A — high value on the current schema (do after S4/S5)
...
|---|---|
| [academic-roadmap/README.md](academic-roadmap/README.md) — S0–S3 done, S4/S5 next | Yes |
...
1. **Fix the teach 500** and add a feature test that GETs `/teach/{offering}`.
2. **Enrich `DemoDataSeeder`** so a client walkthrough has lessons, one quiz, one invoice, one session, one announcement, one issued credential (plan in [demo-accounts.md](demo-accounts.md)).
3. **S5 then S4** as already sequenced.
4. **Operator CRUD**: program/course edit, hold/override forms, waitlist nav, mark complete.
5. **Live payments** when a gateway account exists.
6. **S6 student API**, then advising-lite + reporting pack, then S7 projects.
