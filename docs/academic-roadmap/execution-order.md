...
Confirmed on `main` @ `acb20cb`. The `cursor/s4*`–`s8*` remotes listed in [`s-branch-reconciliation.md`](s-branch-reconciliation.md) are ancestors of main (0 commits ahead). Do not re-run S4–S8 or S6E as new implementation. Still open here: Reverb only.
